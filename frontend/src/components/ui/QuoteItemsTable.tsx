  ...
  return (
    <div className="space-y-2">
      ...
      {items.map((item, index) => (
        <div key={index} className="grid grid-cols-1 gap-2 rounded-lg border border-ink-100 p-2 sm:grid-cols-[1fr_120px_70px_120px_32px] sm:border-0 sm:p-0">
          <input
          ...
          />
          <div className="flex items-center justify-end px-2 text-sm font-medium text-ink-900 sm:px-0">
            {formatCLP(item.unitPrice * item.quantity)}
          </div>
          <button
            type="button"
            onClick={() => removeItem(index)}
            className="flex items-center justify-center text-ink-400 hover:text-red-600 sm:justify-self-end"
          >
            <Trash2 size={16} />
          </button>
        </div>
