...
export default function QuoteFormPage() {
  ...
  const finalPrice = Math.max(0, Number(form.basePrice || 0) - Number(form.discount || 0));

  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.clientId || !form.description || !form.basePrice) {
      toast.error('Cliente, descripción y precio base son obligatorios.');
      return;
    }
    setSaving(true);
    try {
      ...
      <form onSubmit={onSubmit} className="card space-y-4">
        ...
        <div className="rounded-lg bg-ink-50 p-3 text-right">
          ...
          <span className="text-lg font-bold text-ink-900">
            {new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(finalPrice)}
          </span>
        </div>
